...
TEXTS = {
    "English": {
        ...
        "video_desc": "This video explains how Pathfinder AI uses IBM watsonx.ai to help you find the right career.",
        ...
        "footer": "© 2026 GlobalInternet.py – Powered by IBM watsonx.ai",
    ...
    "Français": {
        ...
        "video_desc": "Cette vidéo explique comment Pathfinder AI utilise IBM watsonx.ai pour vous aider à trouver la bonne carrière.",
        ...
        "footer": "© 2026 GlobalInternet.py – Propulsé par IBM watsonx.ai",
    ...
    "Español": {
        ...
        "video_desc": "Este video explica cómo Pathfinder AI usa IBM watsonx.ai para ayudarte a encontrar la carrera correcta.",
        ...
        "footer": "© 2026 GlobalInternet.py – Desarrollado con IBM watsonx.ai",
...
# ================== IBM watsonx.ai & Groq Clients ==================
# Get API keys from secrets
if "IBM_API_KEY" not in st.secrets or "IBM_PROJECT_ID" not in st.secrets:
    st.warning("⚠️ IBM watsonx.ai API keys not found. The app will use Groq as fallback. Add IBM_API_KEY and IBM_PROJECT_ID to Streamlit secrets for best performance.")
    ibm_available = False
else:
    ibm_available = True
...
# ================== AI FUNCTION (IBM first, then Groq) ==================
...
Career Advice:"""
    ...
    # Try IBM watsonx.ai first if available
    if ibm_available:
        try:
            # IBM watsonx.ai API endpoint (text generation)
            url = "https://us-south.ml.cloud.ibm.com/ml/v1/text/generation?version=2023-05-29"
            headers = {
                "Content-Type": "application/json",
                "Authorization": f"Bearer {st.secrets['IBM_API_KEY']}"
            }
            body = {
                "input": prompt,
                "parameters": {
                ...
                "model_id": "google/flan-t5-xxl",  # or use granite model if available
                "project_id": st.secrets["IBM_PROJECT_ID"]
            }
            response = requests.post(url, headers=headers, json=body, timeout=30)
            if response.status_code == 200:
                result = response.json()
                advice = result.get("results", [{}])[0].get("generated_text", "").strip()
                if advice:
                    return advice
        except Exception as e:
            st.warning(f"IBM API error: {e}. Falling back to Groq.")
...
# --- Tab 1: Video Introduction (placeholders) ---
with tab1:
...
with tab2:
    ...
    if submitted:
        if not skills.strip() or not experience.strip() or not goals.strip():
            ...
            st.markdown(advice)
            st.info("Powered by IBM watsonx.ai and Groq")
